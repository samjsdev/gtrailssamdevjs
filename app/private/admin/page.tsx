...
export default function IntakePage() {
  ...
  useEffect(() => {
    client.ping()
      .then((res) => console.log('Appwrite setup verified successfully via ping:', res))
      .catch((err) => console.error('Appwrite verification ping failed:', err));
  }, []);
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/private/admin/edit/${data.slug}`);
    } catch (err: any) {
      ...
      <div className="max-w-2xl w-full flex flex-col items-center">
        ...
        <div className="mt-16 w-full grid grid-cols-1 md:grid-cols-3 gap-6 border-t border-gray-200 pt-10">
          ...
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <h3 className="font-medium text-gray-900">2. Business Details</h3>
            <p className="text-sm text-gray-500">We extract profile details & reviews (images can be scraped in editor)</p>
          </div>
          <div className="flex flex-col items-center text-center space-y-3">
            <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 border border-gray-200">
