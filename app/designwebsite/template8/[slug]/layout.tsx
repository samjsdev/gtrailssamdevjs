...
import { Phone, Grid, MapPin } from 'lucide-react';
...
export default async function DesignStudioLayout({ children, params }: LayoutProps) {
  ...
  return (
    <div className={`${inter.className} min-h-screen bg-[#0B0F19] text-slate-100 selection:bg-[#38BDF8] selection:text-slate-900 scroll-smooth flex flex-col`}>
      {/* Top bar info */}
      <div className="bg-slate-950/80 border-b border-white/5 text-slate-400 py-2 px-8 text-xs relative z-50">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <div className="flex items-center gap-4 text-slate-200">
             <a href={`tel:${clinic.contact?.phone || ''}`} className="flex items-center gap-2 hover:text-[#38BDF8] transition-colors">
               <Phone className="w-3.5 h-3.5 text-[#38BDF8]" /> {clinic.contact?.phone || 'Call'}
             </a>
          </div>
        </div>
      </div>
      ...
      {/* Navbar header */}
      <header className="sticky top-0 z-40 bg-[#0B0F19]/80 backdrop-blur-xl border-b border-white/10 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-8 h-20 flex justify-between items-center">
          <Link href={basePath} className="flex items-center gap-2">
            <h1 className={`${plusJakarta.className} text-xl font-extrabold tracking-tight text-white`}>
              {clinic.name || 'Glass Studio'}
            </h1>
          </Link>
          
          <nav className="hidden lg:flex items-center gap-8 font-medium text-xs uppercase tracking-widest text-slate-400">
           ...
           <Link href="#contact" className="hidden md:flex items-center bg-slate-900/90 border border-slate-700/60 hover:border-[#38BDF8] text-white px-5 py-2.5 rounded-none font-bold uppercase tracking-wider text-xs transition-all">
             Consultation
          </Link>
        </div>
      </header>
      ...
      <footer id="contact" className="bg-[#070a11] border-t border-white/10 pt-16 pb-12 px-8 mt-auto text-slate-400">
        <div className="max-w-7xl mx-auto">
          ...
          {(() => {
            ...
            return (
              <div className="mb-16 rounded-none overflow-hidden border border-slate-700/60 shadow-sm bg-slate-950">
                <iframe
                  src={mapUrl}
                  width="100%"
                  height="350"
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
            <div className="lg:col-span-2 space-y-6">
              <h4 className={`${plusJakarta.className} text-xl font-bold text-white`}>{clinic.name || 'Glass Studio'}</h4>
              <p className="font-light leading-relaxed max-w-sm text-[15px]">{clinic.description || 'Designing visual environments featuring clean metal sections, floating glass, and dynamic backlighting.'}</p>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-4 font-light text-sm">
                <li className="flex items-start gap-3">
                  <MapPin className="w-4 h-4 mt-1 shrink-0 text-[#38BDF8]" />
                  <span>{clinic.address?.full || 'Studio Location'}</span>
                </li>
                <li className="flex items-start gap-3">
                  <Phone className="w-4 h-4 mt-1 shrink-0 text-[#38BDF8]" />
            ...
            <div>
              <h5 className="text-white font-semibold mb-6 uppercase text-xs tracking-wider">QUICK LINKS</h5>
      ...
      {(() => {
        ...
        return (
          <a
            ...
            className="fixed bottom-6 right-6 z-50 w-16 h-16 bg-[#25D366] hover:bg-[#1db954] rounded-none flex items-center justify-center shadow-2xl transition-all hover:scale-110 active:scale-95"
