...
export default function BeforeAfter({ image, beforeImage, afterImage, caption }: BeforeAfterProps) {
  ...
  return (
    <div ref={containerRef} className="w-full">
      <div
        className="relative overflow-hidden rounded-[24px] sm:rounded-[28px] aspect-[16/10] sm:aspect-[16/9.5] shadow-[0_28px_70px_rgba(27,27,27,0.18)] select-none border border-[#1b1b1b]/10 bg-[#1b1b1b]"
        onPointerDown={handlePointerDown}
      >
        {/* Before: Raw Structure (Underneath) */}
        <img
          src={beforeSrc}
          alt="Raw structural frame"
          className="absolute inset-0 w-full h-full object-cover pointer-events-none"
        />

        {/* After: Completed Residence (Clipped from left at pos%) */}
        <img
          src={afterSrc}
        ...
        {/* Badges */}
        <span className="absolute top-3 sm:top-5 left-2 max-[379px]:left-3 sm:left-5 z-[3] text-[8.5px] max-[379px]:text-[7.5px] max-[379px]:tracking-[0.02em] max-[379px]:px-2 max-[379px]:py-1 sm:text-[11.5px] font-extrabold tracking-[0.08em] sm:tracking-[0.14em] uppercase px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-full bg-black/75 text-white backdrop-blur-sm pointer-events-none shadow-md">
          Topography &amp; Shell
        </span>
        <span className="absolute top-3 sm:top-5 right-2 max-[379px]:right-3 sm:right-5 z-[3] text-[8.5px] max-[379px]:text-[7.5px] max-[379px]:tracking-[0.02em] max-[379px]:px-2 max-[379px]:py-1 sm:text-[11.5px] font-extrabold tracking-[0.08em] sm:tracking-[0.14em] uppercase px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-full bg-[#f2a007] text-[#1b1b1b] pointer-events-none shadow-md">
          Completed Landmark
        </span>
        ...
        <div
          ...
          style={{ left: `${pos}%` }}
        >
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[46px] h-[46px] sm:w-[52px] sm:h-[52px] rounded-full bg-[#0e5a43] grid place-items-center text-white font-extrabold text-[16px] shadow-[0_8px_24px_rgba(14,90,67,0.4)] border-2 border-white">
            ↔
          </div>
        </div>
      ...
      {caption && (
        <p className="text-center text-[13px] text-[#6b6660] mt-4 font-medium tracking-wide">
          {caption}
        </p>
      )}
    </div>
