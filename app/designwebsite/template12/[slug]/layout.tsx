...
export default async function Template12Layout({ children, params }: LayoutProps) {
  ...
  const waText = `Hi, I'm interested in an architectural consultation with ${cleanName || 'your studio'}!`;
  ...
  return (
    <div className={`${jakarta.className} ${jakarta.variable} ${bricolage.variable} min-h-screen flex flex-col bg-white text-[#1b1b1b] selection:bg-[#f2a007] selection:text-[#1b1b1b] scroll-smooth antialiased`}>
      ...
      <div className="bg-[#0e5a43] text-white text-center text-[13px] font-semibold px-4 py-2.5 tracking-[0.02em]">
        <b className="text-[#f2a007]">Architectural consultation</b> — 3D massing + municipal bylaw review for your plot.{' '}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1.3fr] gap-10 pb-11 border-b border-white/10">
            <div>
              ...
              <p className="text-[13.5px] leading-[1.75] mt-4 max-w-[290px] font-medium">
                {cleanDesc || 'Sustainable residential architecture, passive solar design, and turnkey civil execution.'}
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="grid gap-2.5">
                <li><Link href={`${basePath}/gallery`} className="text-[14px] font-medium hover:text-[#f2a007] transition-colors">Architecture Gallery</Link></li>
                <li><Link href={`${basePath}/services`} className="text-[14px] font-medium hover:text-[#f2a007] transition-colors">Disciplines</Link></li>
            ...
            <div>
              <h4 className="text-white text-[12px] font-extrabold tracking-[0.24em] uppercase mb-4.5">Disciplines</h4>
              <ul className="grid gap-2.5">
                {(servicesList.length ? servicesList.slice(0, 4) : ['Turnkey Villas', 'Passive Architecture', 'Structural BIM', 'Bylaw Sanctions']).map((svc) => (
                  <li key={svc}>
