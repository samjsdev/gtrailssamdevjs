...
import { cleanClinicName, cleanArchitectureDescription, cleanArchitectureServices } from '@/lib/copyCleaner';
...
export default async function Template11Layout({ children, params }: LayoutProps) {
  ...
  const initial = (cleanName || 'A').charAt(0).toUpperCase();
  ...
  const waText = `Hi, I'm interested in booking an architectural consultation with ${cleanName || 'your architects'}!`;
          ...
          <div className="flex gap-6 min-w-0">
            {address && (
              <span className="truncate">
                <b className="text-white font-medium">Office:</b> {address}
              </span>
            )}
            <span className="hidden md:inline shrink-0">
              <b className="text-white font-medium">Hours:</b> Mon–Sat, 9:30 AM – 7 PM
            </span>
          </div>
          {phone && (
      ...
      <footer className="bg-[#211a13] text-[#f6f1e8]/70 px-6 lg:px-7 pt-16 pb-9">
        <div className="max-w-7xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr] gap-11 pb-12 border-b border-[#f6f1e8]/15">
            <div>
              ...
              <p className="text-[13.5px] font-light leading-relaxed mt-4.5 max-w-[300px]">
                {cleanDesc || 'We design and build modern luxury villas, residential homes, and commercial buildings across the city with quality materials and a 10-year warranty.'}
              </p>
            </div>

            <div>
              <h4 className="text-[12px] tracking-[0.3em] uppercase text-[#c9ab7c] mb-5 font-medium">Company</h4>
              <ul className="grid gap-3">
                <li><Link href={`${basePath}/about`} className="text-[14px] font-light hover:text-[#c9ab7c] transition-colors">About Us</Link></li>
                <li><Link href={`${basePath}/gallery`} className="text-[14px] font-light hover:text-[#c9ab7c] transition-colors">Our Projects</Link></li>
                <li><Link href={`${basePath}/services`} className="text-[14px] font-light hover:text-[#c9ab7c] transition-colors">Services</Link></li>
                <li><Link href={`${basePath}/contact`} className="text-[14px] font-light hover:text-[#c9ab7c] transition-colors">Contact</Link></li>
              </ul>
            </div>

            <div>
              <h4 className="text-[12px] tracking-[0.3em] uppercase text-[#c9ab7c] mb-5 font-medium">Disciplines</h4>
              <ul className="grid gap-3">
            ...
            <div>
              ...
              <address className="not-italic text-[14px] font-light leading-[1.9]">
                <b className="text-white font-medium">{cleanName || 'Our Office'}</b>
                <br />
          ...
          <div className="flex flex-wrap justify-between gap-3.5 pt-7 text-[12px] tracking-[0.08em] text-[#f6f1e8]/45">
            <span>© {new Date().getFullYear()} {cleanName || 'Architects & Builders'}. All rights reserved.</span>
            <span>Villas, residential homes &amp; commercial buildings across {city}</span>
          </div>
        </div>
      </footer>
