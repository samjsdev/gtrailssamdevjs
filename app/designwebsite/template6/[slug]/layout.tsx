...
export default async function DesignStudioLayout({ children, params }: LayoutProps) {
  ...
  return (
    <div className={`${lato.className} min-h-screen bg-[#1a1a1a] text-zinc-300 selection:bg-zinc-800 selection:text-white scroll-smooth flex flex-col`}>
      {/* Elegant Header */}
      <header className="absolute top-0 w-full z-50 px-6 py-8 border-b border-white/10 bg-gradient-to-b from-black/60 to-transparent">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link href={basePath} className="flex flex-col">
            <span className={`${playfair.className} text-2xl md:text-3xl font-medium tracking-wide text-white uppercase`}>
              {clinic.name || "Studio"}
            </span>
          </Link>
          
          <nav className="hidden md:flex items-center gap-10 text-[11px] uppercase tracking-[0.25em] text-zinc-300 font-bold">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm font-light text-zinc-400">
                <li><Link href="#services" className="hover:text-white transition">Expertise</Link></li>
                <li><Link href="#about" className="hover:text-white transition">Our Story</Link></li>
                <li><Link href="#gallery" className="hover:text-white transition">Selected Works</Link></li>
              </ul>
            </div>
          </div>
