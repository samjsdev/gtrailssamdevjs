...
export default function TreatmentShowcase({ data }: { data?: any }) {
  ...
  const sectionTitle = data?.gallery?.exploreTitle || 'Our Core Services';
  const services = data?.business?.services?.length ? data.business.services : ['Turnkey Modern Villas', '3D Elevation Design', 'CMDA Plan Approvals', 'Commercial Buildings', '100% Vaastu Planning'];
