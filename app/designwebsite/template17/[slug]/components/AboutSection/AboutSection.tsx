...
import { getArchitectureServiceData, getArchitectureServiceSummary } from '@/lib/architectureContent';
import {
...
export default function AboutSection({ data }: { data?: any }) {
  ...
  const clinicName = clinic.name || 'Architectural Design Studio';
  const city = clinic.address?.city || 'Chennai';
  const clinicDesc = cleanArchitectureDescription(about.description || clinic.description, clinic.name, city);
  const aboutSubtitle = about.subtitle || 'Who We Are';
  const aboutTitle = about.title || 'Modern Villas & Quality Residential Construction in Chennai.';
  const expertiseTitle = about.expertiseTitle || 'Our Core Services';
  const expertiseSubtitle = about.expertiseSubtitle || 'What We Deliver';
  
  const leaderSectionTitle = leader.title || 'Studio Leadership';
  const leaderName = doctor.name || 'Principal Architect';
  const leaderFirstName = leaderName.split(' ')[0] || 'Principal';
  const leaderRole = cleanArchitectureSpecialization(doctor.specialization);
  const leaderExp = doctor.experience || '14+';
  const leaderQuote = leader.quote || '"Great architecture comes from combining beautiful design with strong, lasting engineering."';
  const leaderBio = doctor.bio || 'With over 14+ years of hands-on architectural and construction experience in Chennai, we lead projects from initial 3D drawings and CMDA plan sanctions to turnkey construction and key handover.';
  const leaderImg = data?.media?.otherImages?.[0] || '/images/architecture/principal-architect.webp';

  const cleanServices = cleanArchitectureServices(business.services);
  const servicesList = (cleanServices.length ? cleanServices : [
    'Modern Villa Construction', 'Residential Home Building', '3D Elevation Design', 'Commercial Buildings'
  ]).slice(0, 4).map((svc: string) => {
    const svcData = getArchitectureServiceData(svc);
    return {
      title: svc,
      icon: (svcData as any)?.icon || '🏛️',
      description: svcData?.description || getArchitectureServiceSummary(svc) || 'Quality architectural design and reliable turnkey construction.'
    };
  });
        ...
        <div className="what-section">
           <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
               <div>
                ...
                <Link href={`${basePath}/services`} className="text-[var(--accent)] hover:text-[var(--accent-2)] transition-colors font-medium flex items-center gap-2">
           ...
           <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-16 lg:gap-24 items-center">
              ...
              <div className="flex flex-col">
                 ...
                 <div className="leader-text flex flex-wrap items-center justify-between gap-6 border-t border-[var(--border)] pt-8">
                    ...
                    <Link href={`${basePath}/contact`} className="px-8 py-3.5 bg-[var(--text)] text-[var(--bg)] rounded-full font-medium transition-transform hover:scale-105 shadow-lg">
                       Book Consultation
                    </Link>
                 </div>
              </div>
