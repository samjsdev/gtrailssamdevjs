      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-16">
            ...
            <Link href={basePath || '/'} className="inline-block">
              <div className="flex items-center gap-3">
                 <div className="relative w-10 h-10 flex items-center justify-center bg-blue-600 rounded-xl shadow-lg shadow-blue-600/20">
                    <span className="text-white font-bold text-xl">{(data?.clinic?.name || data?.navbar?.logo?.text || "A")[0]}</span>
                 </div>
                <span className="text-2xl font-bold text-slate-900 tracking-tight">{data?.clinic?.name || data?.navbar?.logo?.text || "Architectural Atelier"}</span>
              </div>
            </Link>
            <p className="text-slate-600 leading-relaxed font-medium max-w-sm">
              {data?.clinic?.description || "Architectural Atelier. Designing and building generational residences across Chennai with licensed architects and structural engineers."}
            </p>
          ...
          <div className="lg:col-span-3 md:col-span-1">
            ...
            <ul className="flex flex-col gap-4">
              {[
                { label: 'Architectural Concept & 3D BIM', href: `${basePath}/services` },
                { label: 'CMDA & DTCP Sanctions', href: `${basePath}/services` },
                { label: 'Structural Engineering', href: `${basePath}/services` },
                { label: 'Turnkey Villa Construction', href: `${basePath}/services` },
              ].map((item) => (
                <li key={item.label}>
                  <Link 
                    href={item.href} 
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              {(() => {
                const phone = data?.contact?.phone || '+91 97513 96117';
                return (
              ...
              {(() => {
                const email = data?.contact?.email || 'hello@architecturalatelier.in';
                return (
        ...
        <div className="pt-8 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-slate-500 text-sm font-medium">
            © {currentYear} Architectural Atelier. All rights reserved.
          </p>
          <div className="flex gap-6">
            <Link href={`${basePath}/privacy`} className="text-slate-500 hover:text-blue-600 text-sm font-medium transition-colors">Privacy Policy</Link>
            <Link href={`${basePath}/terms`} className="text-slate-500 hover:text-blue-600 text-sm font-medium transition-colors">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
