...
export default function HighlightsCarousel({ data }: { data?: any }) {
  ...
  const mainHeading = data?.projects?.title || "Real homes. \n Real quality.";
  ...
  const slides: Slide[] = useMemo(
    () => {
      const defaultTitles = [
        ...
        "OMR Commercial Complex.",
        "Anna Nagar Luxury Duplex.",
        "Independent Family Residence.",
        "Heat-Proof Cool Brick Home.",
        "Contemporary Glass Villa."
      ];
      const defaultSubtitles = [
        "Spacious open balconies and private swimming pool on ECR.",
        "Open-to-sky courtyard with natural cooling and 100% Vaastu.",
        "Modern office building with maximum floor space and parking.",
        "Multi-generational G+2 home with private lift provision.",
        "Quality Tata Tiscon steel and solid foundation on individual plot.",
        "Hollow clay bricks keeping rooms naturally cool in summer.",
        "Large windows with natural daylight and open garden views."
      ];
      const defaultImages = [
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-16">
        <div className="flex flex-col md:flex-row items-baseline justify-between mb-12 md:mb-20 gap-4">
          ...
          <Link href={`${basePath}/gallery`} className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-colors shadow-lg hover:shadow-blue-500/30">
