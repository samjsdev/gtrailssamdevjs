...
const CTASection = ({ data }: { data?: any }) => {
  ...
  const title = data?.cta?.title || 'Ready to commission your landmark residence?';
  const description = data?.cta?.description || 'Consult with our principal architects and civil engineers in Chennai. Bring your plot dimensions to receive statutory CMDA setback guidance, 3D massing, and a frozen line-item BOQ.';
          ...
          <div className="relative z-10 grid md:grid-cols-[1.15fr_0.85fr] gap-8 xl:gap-12 items-start">
            <div>
              ...
              <div className="cta-actions flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
                <Link href={`${basePath}/contact`} className="group inline-flex justify-center items-center gap-2 px-8 py-4 font-semibold rounded-xl bg-[var(--accent)] text-white hover:bg-[var(--accent)]/90 transition-all duration-200 shadow-lg shadow-[var(--accent)]/25">
                  {buttonText}
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-2)]/50 p-6 md:p-8 backdrop-blur-sm">
              ...
              <ul className="space-y-4 text-sm text-[var(--muted)]">
                <li className="flex items-start gap-4">
                  ...
                  <span>Clear explanation of design options, structural systems, and expected outcomes.</span>
                </li>
                <li className="flex items-start gap-4">
                  <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[var(--accent-2)] flex-shrink-0"></span>
                  <span>Transparent pricing discussion before any construction starts.</span>
                </li>
                <li className="flex items-start gap-4">
                  <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[var(--accent-2)] flex-shrink-0"></span>
                  <span>Project plan coordinated by the right architect and engineer for your site.</span>
                </li>
              </ul>
            </div>
          </div>
