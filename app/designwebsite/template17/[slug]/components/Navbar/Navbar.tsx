      ...
      >
        ...
        >
          <div className="relative w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-blue-600 rounded-xl shadow-lg shadow-blue-600/20">
             <span className="text-white font-bold text-lg md:text-xl">{(data?.clinic?.name || navbarData?.logo?.text || "A")[0]}</span>
          </div>
          <span className="text-xl md:text-2xl font-bold">{data?.clinic?.name || navbarData?.logo?.text || "Atelier"}</span>
        </Link>
