...
export default async function Template4Layout({ children, params }: LayoutProps) {
  ...
  const waText = `Hello ${cleanName || 'there'}, I'd like to book a private design consultation.`;
      ...
      <footer className="bg-[#17130f] text-white pt-[76px]">
        <div className="max-w-[1240px] mx-auto px-[30px]">
          <div className="grid sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1.3fr] gap-12 pb-14">
            <div>
              <Link href={basePath} className="flex flex-col leading-[1.05]">
                ...
                <small className="text-[9.5px] tracking-[0.48em] uppercase text-white/50">Curated Home Interiors</small>
              </Link>
              <p className="text-[14px] text-white/60 leading-[2.1] max-w-[300px] mt-4">
                {cleanDesc || `A boutique interior atelier in ${city}. Signature homes, one standard of craft on every door.`}
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="grid">
                ...
                <li><Link href={`${basePath}/services`} className="text-[14px] text-white/60 leading-[2.1] hover:text-[#d9c49a] transition-colors">Services</Link></li>
                <li><Link href={`${basePath}/gallery`} className="text-[14px] text-white/60 leading-[2.1] hover:text-[#d9c49a] transition-colors">Homes</Link></li>
                <li><Link href={`${basePath}/contact`} className="text-[14px] text-white/60 leading-[2.1] hover:text-[#d9c49a] transition-colors">Visit</Link></li>
              </ul>
            </div>

            <div>
              <h4 className="text-[11px] font-semibold tracking-[0.26em] uppercase text-[#b08d4f] mb-5">Services</h4>
              <ul className="grid">
                {(servicesList.length ? servicesList.slice(0, 5) : ['Full Home Interiors', 'Modular Kitchens', 'Bedrooms & Wardrobes', 'Renovation']).map((svc) => (
                  <li key={svc}>
          ...
          <div className="border-t border-white/10 py-[22px] flex flex-wrap justify-between gap-3.5 text-[12px] text-white/40">
            ...
            <span>Bespoke interiors, crafted in {city}</span>
          </div>
        </div>
      </footer>
