...
export default async function Template13Layout({ children, params }: LayoutProps) {
  ...
  const waText = `Hi, I'm interested in an architectural consultation with ${cleanName || 'your atelier'}!`;
        ...
        <div className="max-w-[1220px] mx-auto flex justify-between items-center gap-4.5 font-semibold">
          <span className="flex items-center gap-2 min-w-0">
            ...
            <span className="truncate">
              {city}&rsquo;s licensed architectural practice — <b className="text-[#f4b942]">{rating}★ on Google</b>
            </span>
          </span>
          {address && (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1.3fr] gap-10 pb-12">
            <div>
              ...
              <p className="text-[13.5px] text-white/70 leading-[2.05] max-w-[280px] mt-3.5">
                {cleanDesc || `Contemporary architectural practice, modern luxury villas, and turnkey civil execution across ${city}.`}
              </p>
            </div>

            <div>
              <h4 className="text-[11.5px] font-extrabold tracking-[0.2em] uppercase text-[#f4b942] mb-4.5">Offerings</h4>
              {(servicesList.length ? servicesList.slice(0, 5) : ['Turnkey Architectural Build', '3D BIM Schematics', 'Structural RCC Engineering', 'Municipal Sanctions']).map((svc) => (
