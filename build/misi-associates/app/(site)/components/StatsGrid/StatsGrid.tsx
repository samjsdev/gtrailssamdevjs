...
export default function StatsGrid({ data }: { data?: any }) {
  ...
  const cards = data?.comprehensive?.cards || [
    { tag: 'PLANNING', title: 'Architectural Design', description: 'Concept planning, elevations, technical drawings, and coordinated design development.' },
    { tag: 'EXECUTION', title: 'Civil Construction', description: 'Quality-led execution with structured site supervision and material coordination.' },
    { tag: 'COORDINATION', title: 'Project Management', description: 'Milestones, vendors, documentation, and communication managed from start to finish.' },
    { tag: 'HANDOVER', title: 'Turnkey Interiors', description: 'Interior detailing, installation, styling, and final quality review.' }
  ];
    ...
    <section id="stats" ref={containerRef} className="py-16 md:py-24 px-6 relative overflow-visible bg-gradient-to-b from-white to-[var(--bg)]">
      ...
      <div className="absolute top-0 left-10 -translate-y-1/2 z-20 hidden lg:block">
         <div className="glass-card px-6 py-2 rounded-full transform -rotate-6 shadow-lg bg-white/40">
            <span className="font-bold text-blue-900 tracking-wide">ARCHITECTURAL PLANNING</span>
         </div>
      </div>
      <div className="absolute top-20 right-10 z-20 hidden lg:block">
         <div className="glass-card px-6 py-2 rounded-full transform rotate-12 shadow-lg bg-white/40">
            <span className="font-bold text-blue-900 tracking-wide">TURNKEY EXECUTION</span>
         </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 auto-rows-auto md:auto-rows-[300px]">
          ...
          <div className="stat-card col-span-1 md:col-span-8 relative group rounded-[32px] md:rounded-[40px] overflow-hidden glass-card border-[3px] border-white/50 bg-gradient-to-br from-blue-50 to-blue-100/50 min-h-[300px]">
             ...
             <div className="absolute bottom-0 left-0 p-8 md:p-10 z-20">
                <span className="inline-block px-4 py-1 rounded-full bg-blue-600 text-white text-xs font-bold mb-4">{cards[0].tag}</span>
                <h3 className="text-2xl md:text-3xl font-bold text-slate-800 mb-2">{cards[0].title}</h3>
                <p className="text-slate-500 max-w-sm text-sm md:text-base">{cards[0].description}</p>
             </div>
          ...
          <div className="stat-card col-span-1 md:col-span-4 row-span-1 md:row-span-2 relative group rounded-[32px] md:rounded-[40px] overflow-hidden glass-card border-[3px] border-white/50 bg-slate-900 min-h-[350px]">
             ...
             <div className="absolute inset-x-0 bottom-0 p-8 pt-20 z-20">
                <span className="inline-block px-4 py-1 rounded-full bg-indigo-500 text-white text-xs font-bold mb-4">{cards[1].tag}</span>
                <h3 className="text-3xl md:text-4xl font-bold text-white mb-2 leading-none">{cards[1].title}</h3>
                <p className="text-slate-200 mt-4 text-sm font-medium">{cards[1].description}</p>
                <div className="mt-8">
                   <a href="/services" aria-label="View Misi Associates services" className="w-12 h-12 rounded-full bg-white/20 backdrop-blur text-white flex items-center justify-center hover:bg-white hover:text-indigo-600 transition-all shadow-lg">
          ...
          <div className="stat-card col-span-1 md:col-span-4 relative group rounded-[32px] md:rounded-[40px] overflow-hidden glass-card border-[3px] border-white/50 bg-blue-50 min-h-[250px]">
             ...
             <div className="p-8 h-full flex flex-col justify-between relative z-10">
                ...
                <div>
                   <h3 className="text-xl font-bold text-slate-800 mb-1">{cards[2].title}</h3>
                   <p className="text-sm text-slate-600 font-medium">{cards[2].description}</p>
                </div>
             </div>
          </div>
          ...
          <div className="stat-card col-span-1 md:col-span-4 relative group rounded-[32px] md:rounded-[40px] overflow-hidden bg-slate-800 text-white shadow-xl shadow-blue-500/20 min-h-[250px]">
             ...
             <div className="p-8 h-full flex flex-col justify-between relative z-20">
                <div className="text-blue-200 font-medium tracking-widest text-xs uppercase">{cards[3].tag}</div>
                <div>
                   <h3 className="text-2xl font-bold mb-2">{cards[3].title}</h3>
                   <p className="text-sm text-slate-200 font-medium max-w-sm">{cards[3].description}</p>
