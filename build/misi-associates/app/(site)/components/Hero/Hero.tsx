...
const CARD_CONFIGS = [
  {
    ...
    inner: "relative w-[120px] sm:w-[170px] md:w-[230px] aspect-[4/3] rounded-[12px] md:rounded-[24px] opacity-70 shadow-xl border-2 md:border-4 border-white/80 overflow-hidden"
  },
  {
    wrapper: "absolute top-[60%] md:top-1/2 left-[25%] md:left-[25%] -translate-x-1/2 -translate-y-1/2 z-10 -rotate-12",
    inner: "relative w-[170px] sm:w-[230px] md:w-[340px] aspect-[4/3] rounded-[16px] md:rounded-[32px] opacity-100 shadow-2xl border-4 md:border-8 border-white overflow-hidden"
  },
  {
    wrapper: "absolute top-[50%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 rotate-0",
    inner: "relative w-[310px] sm:w-[440px] md:w-[680px] aspect-[4/3] rounded-[24px] md:rounded-[32px] opacity-100 shadow-2xl border-4 md:border-8 border-white overflow-hidden"
  },
  {
...
export default function Hero() {
  ...
  useGSAP(() => {
    let clearStylesTimer: number | undefined;
    ...
    const tl = gsap.timeline({
      ...
      onComplete: () => {
        ...
        clearStylesTimer = window.setTimeout(() => {
    ...
    return () => {
      tl.kill();
      if (clearStylesTimer !== undefined) window.clearTimeout(clearStylesTimer);
      if (startRotationTimer !== undefined) window.clearTimeout(startRotationTimer);
      if (rotationInterval !== undefined) window.clearInterval(rotationInterval);
    };
               ...
               return (
                   <div key={index} className={`hero-moving-card ${config.wrapper} ${isReady ? 'transition-all duration-1000 ease-in-out' : ''}`}>
                       <div className={`hero-img-inner ${config.inner} ${isReady ? 'transition-all duration-1000 ease-in-out' : ''} group cursor-pointer`}>
                           ...
                           <Image 
                               src={src} 
                               alt={`${data?.clinic?.name || 'Misi Associates'} project ${index + 1}`} 
                               fill 
                               className="object-cover transition-transform duration-700 group-hover:scale-110"
                               preload={index === 2}
                               loading="eager"
                               sizes="(max-width: 640px) 310px, (max-width: 1024px) 440px, 680px"
                           />
                       </div>
                   </div>
               );
