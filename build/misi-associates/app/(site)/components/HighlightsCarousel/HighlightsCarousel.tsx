...
export default function HighlightsCarousel({ data }: { data?: any }) {
  ...
  const slides: Slide[] = useMemo(
    () => {
      ...
      const titles = data?.projects?.titles?.length ? data.projects.titles : defaultTitles;
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-16">
        <div className="flex flex-col md:flex-row items-baseline justify-between mb-12 md:mb-20 gap-4">
          ...
          <Link href="/gallery" className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-colors shadow-lg hover:shadow-blue-500/30">
          ...
          >
            {slides.map((slide, idx) => (
              <div
                ...
                onClick={() => goTo(idx)}
              >
                <div className={`glass-card relative aspect-[4/3] rounded-[48px] overflow-hidden transition-all duration-500 border-[4px] border-white
                  ${idx === activeIndex 
