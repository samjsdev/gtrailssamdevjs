...
const CTASection = ({ data }: { data?: any }) => {
  ...
  const description = data?.cta?.description || 'Speak with our team about your site, scope, budget, and timeline, and get a clear path from design through completion.';
  const buttonText = data?.cta?.buttonText || 'Book a Consultation';
  const phone = data?.clinic?.contact?.phone || '+91 95436 02232';
  const eyebrow = data?.cta?.eyebrow || 'Next Step';
  const promiseTitle = data?.cta?.promiseTitle || 'What to Expect';
  const promiseItems = data?.cta?.promiseItems || [
    'A clear discussion of your goals, project scope, and priorities.',
    'Practical guidance on design, execution, budget, and timeline.',
    'A coordinated delivery plan from concept through handover.'
  ];
      ...
      <div className="max-w-[1100px] mx-auto">
        <div className="cta-shell relative p-8 md:p-12 lg:p-16 rounded-[32px] bg-[var(--surface)] border border-[var(--border)] overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 grid md:grid-cols-[1.15fr_0.85fr] gap-8 xl:gap-12 items-start">
            <div>
              <p className="font-mono text-xs tracking-[0.2em] uppercase text-[var(--accent)] mb-4">{eyebrow}</p>
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-2)]/50 p-6 md:p-8 backdrop-blur-sm">
              <p className="font-mono text-[11px] tracking-[0.16em] uppercase text-[var(--muted-2)] mb-5">{promiseTitle}</p>
              <ul className="space-y-4 text-sm text-[var(--muted)]">
                {promiseItems.map((item: string) => (
