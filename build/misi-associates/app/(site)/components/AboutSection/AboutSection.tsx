...
export default function AboutSection({ data }: { data?: any }) {
  ...
  const leaderImg = leader.image || doctor.images?.[0] || data?.media?.otherImages?.[0] || '/images/stock/bbb7f0e7.webp';
  const leaderBadgeLabel = leader.badgeLabel || 'Years Design Experience';
  const leaderSignatureName = leader.signatureName || leaderFirstName;
  const leaderSignatureLabel = leader.signatureLabel || 'Design Leadership';
  const leaderCtaLabel = leader.ctaLabel || 'Book Consultation';
  ...
  ]).slice(0, 4).map((svc: string, index: number) => {
    const svcData = getInteriorServiceData(svc);
    return {
      title: svc,
      icon: (svcData as any)?.icon || '✨',
      description: business.serviceDescriptions?.[index] || svcData?.description || getInteriorServiceSummary(svc) || 'Transforming spaces into personalized environments.'
    };
  });
  ...
  useGSAP(() => {
    ...
    gsap.fromTo('.who-reveal', 
      { opacity: 0, y: 30 },
      {
        ...
        scrollTrigger: { trigger: '.who-section', start: 'top 80%', toggleActions: 'play none none none', once: true }
      }
    );
    ...
    gsap.fromTo('.service-card',
      { opacity: 0, y: 40 },
      {
        ...
        scrollTrigger: { trigger: '.what-section', start: 'top 75%', toggleActions: 'play none none none', once: true }
      }
    );
    ...
    gsap.fromTo('.leader-img',
      { opacity: 0, scale: 0.95 },
      {
        ...
        scrollTrigger: { trigger: '.leader-section', start: 'top 75%', toggleActions: 'play none none none', once: true }
      }
    );
           ...
           <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-16 lg:gap-24 items-center">
              ...
              <div className="leader-img relative mx-auto lg:mx-0 w-full max-w-[500px]">
                 ...
                 <div className="absolute -bottom-6 -right-6 lg:-bottom-10 lg:-right-10 z-20">
                   <div className="glass-card p-6 rounded-3xl shadow-xl border border-white/60 backdrop-blur-xl text-center">
                      ...
                      <span className="text-xs font-semibold text-[var(--muted)] uppercase tracking-wider">{leaderBadgeLabel}</span>
                   </div>
                 </div>
              </div>
              ...
              <div className="flex flex-col">
                 ...
                 <div className="leader-text flex flex-wrap items-center justify-between gap-6 border-t border-[var(--border)] pt-8">
                    <div>
                      <span className="block font-serif text-2xl text-[var(--text)] italic opacity-70">{leaderSignatureName}</span>
                      <span className="text-xs text-[var(--muted)] tracking-widest uppercase mt-1 block">{leaderSignatureLabel}</span>
                    </div>
