      ...
      <div className="w-full max-w-5xl transition-all duration-300">
        <div className={`flex flex-col bg-white/80 backdrop-blur-md border border-stone-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] text-stone-900 transition-all duration-300 overflow-hidden ${
          ...
          <div className="flex items-center justify-between px-6 py-3.5 w-full">
            <Link href={basePath || '/'} className="flex items-center gap-3 hover:opacity-85 transition-opacity shrink-0">
              <img src={`${basePath || ''}/waringlogo.webp`} alt={clinic.name || "Studio Logo"} className="h-10 md:h-12 w-auto object-contain rounded-full mix-blend-multiply" />
              <span className="text-lg md:text-xl font-thin tracking-widest uppercase text-stone-900">{clinic.name || "Studio"}</span>
            </Link>
